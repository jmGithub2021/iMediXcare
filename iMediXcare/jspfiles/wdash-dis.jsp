...
<%
	...
	Vector v = (Vector)dpStats.getDiseaseData(vcent,vstart,vto);
	//out.println( v.size() );
	JsonArray data = new JsonArray();
	JsonArray row = new JsonArray();
	row.add(new JsonPrimitive("Disease"));
	...
	for (int i = 0; i < v.size(); i++){
		...
		if (Integer.parseInt(innerObj.getValue("nop"))>25) {
			row = new JsonArray();
			row.add(new JsonPrimitive( innerObj.getValue("class") ));
