...
<%
		...
		// 1st object 'dataobj' and 2nd object 'Vector' ;
			...
			//out.println("IMG :'"+imgname+"'");
			//out.println("MOV :'"+movname+"'");
...
String ty="",id="",sl="";
	...
	if (ty.equalsIgnoreCase("k00") || ty.equalsIgnoreCase("k01") || ty.equalsIgnoreCase("k02")) {
		String redirectto = "disantipres.jsp?" +  request.getQueryString();
		response.sendRedirect(response.encodeRedirectURL(redirectto));
	}
	if (ty.equalsIgnoreCase("ai0")) {
		String redirectto = "ai0.jsp?" +  request.getQueryString();
		response.sendRedirect(response.encodeRedirectURL(redirectto));
	}
...
boolean found=false;
	...
	String wprn1 ="";
	 ...
	 wprn1 = "<a class='btn btn-default btn-sm' href='writevalall.jsp?id="+id+"&ty="+ty+"&dt="+dt1+"&sl="+sl+"' Style='text-decoration: none; Color:brown; font-weight: bold; font-size: 10' >";
	out.println("<tr><td><table BORDER=1>");

    out.println("<td Align=RIght VAlign=Bottom style='background-color=#C8DCE1'>"+ wprn1 +"&nbsp;&nbsp;All Fields&nbsp;&nbsp;</A></td></table></td></tr>");

	try {
	//out.println(ty+" : "+id+" : "+dt+" : "+sl);

	Object res=ddinfo.DisplayFrm(ty.toLowerCase(),id,dt1,sl);
	...
	if(res instanceof String){
		...
		if((char) i != '\n')
			...
			//System.out.println(str);		//contains a single line of the html file
			...
			if(got != -1)			// the line contains the field name
			{
				...
				if ((data.equals("") && (input != -1 && chkbx==-1)) ||(data.equals("0.0") && (input != -1 && chkbx==-1)) || (data.equals("0") && (input != -1 && chkbx==-1)) || fname.equals("nofimg") )
				{
				 //out.print("&nbsp;"+ data);
				 start++;
				}
			  ...
			  if(ty.equalsIgnoreCase("a41")){
				...
				String imgsrc="<IMG class='img-responsive' SRC='./../images/pain/"+pain+"' WIDTH='80' HEIGHT='80' ALT='Pain'>";
...
//i00-0#;
if(imgname.equalsIgnoreCase("") && imgname.equalsIgnoreCase(""))
{
...
if(imgname.length()>4){
	...
	for(int i=0;i<arr.length;i++){
		...
		//out.println("<TD Width=30 Height=30 Valign=Bottom><A Href='displayimg.jsp?id="+patid+"&type="+fty+"&ser="+arr1[1]+"&dt="+dt1+"' onclick=\"NewWindow(this.href,'AttachFiles','520','450','yes','CENTER');return false\" >"+wprn+"</A></TD>");
...
if(movname.length()>4){
	...
	for(int i=0;i<array.length;i++){
		...
		String wprn="<IMG SRC='../images/video.jpg' Width=30 Height=30 >";
		//out.println("<TD Width=30 Height=30 Valign=Bottom><A Href='viewmovie.jsp?id="+patid+"&ty="+fty+"&sl="+array1[1]+"&dt="+dt1+"' Target=fullform>"+wprn+"</A></TD>");

		out.println("<TD Width=30 Height=30 Valign=Bottom><A Href='viewmovie.jsp?id="+patid+"&ty="+fty+"&sl="+array1[1]+"&dt="+dt1+"' onclick=\"NewWindow(this.href,'AttachFiles','520','450','yes','CENTER');return false\">"+wprn+"</A></TD>");
